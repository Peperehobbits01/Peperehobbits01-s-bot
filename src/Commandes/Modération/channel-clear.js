...
module.exports = {
	...
	options: [
		{
			...
			type: "channel",
			name: "salon",
			description: "Le salon dans lequel tu veux effacer les messages.",
			required: false,
			autocomplete: false
		}
	],
	...
	async run(bot, message, args) {
		...
		if (parseInt(number) <= 0 || parseInt(number) > 100) return message.reply("Le bot n'est pas capable de gérer un aussi grand nombre de suppressions !")
		...
		try {
			...
			await message.channel.send({content: `Je n'ai pas pu tout supprimer car certains messages datent de plus de 14 jours !`})
		}
	}
}
