...
module.exports = {
    ...
    async run(bot, message) {
        ...
        if (leaderboard.length <= 5) {
                ...
                // --- Column 1: Ranks 1 through 5 (Indices 0 to 4) ---
            for (let i = 0; i < Math.min(leaderboard.length, 5); i++) {
                    ...
                    // 1. Draw Status Indicator first (as requested)
                    ctx.beginPath();
                    ...
                    // Draw Avatar second (using clipping to keep it rounded)
                    ctx.save();
                    ctx.beginPath();
                    ctx.arc(104, (74 + ((i) * 128)), 42.5, 0, Math.PI * 2, true);
                    ctx.closePath();
                ...
                // --- Column 2: Ranks 6 through 10 (Indices 5 to 9) ---
            const column2Count = Math.min(leaderboard.length - 5, 5); // Max 5 users (indices 5 through 9)
            const column2X = 744; // X position for the second column

            for (let i = 0; i < column2Count; i++) {
                const leaderboardIndex = 5 + i; // Calculate actual index in leaderboard array
